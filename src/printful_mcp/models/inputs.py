...
class ListCatalogProductsInput(BaseModel):
    ...
    techniques: Optional[str] = Field(
        default=None, description="Comma-separated techniques (dtg, embroidery, etc.)"
    )
...
class GetProductAvailabilityInput(BaseModel):
    ...
    product_id: int = Field(..., description="Catalog product ID")
    techniques: Optional[str] = Field(
        default=None, description="Comma-separated techniques to filter"
    )
    format: Literal["markdown", "json"] = Field(default="markdown", description="Output format")
...
class ListCategoriesInput(BaseModel):
    ...
    format: Literal["markdown", "json"] = Field(default="markdown", description="Response format")
...
class GetSizeGuideInput(BaseModel):
    ...
    unit: Optional[str] = Field(
        default=None, description="Measurement unit: 'inches' or 'cm'. Omit for the API default."
    )
    format: Literal["markdown", "json"] = Field(default="markdown", description="Response format")
...
class GetOrderInput(BaseModel):
    ...
    order_id: str = Field(
        ..., description="Order ID or external ID (prefix with @ for external ID)"
    )
    format: Literal["markdown", "json"] = Field(default="markdown", description="Output format")
...
class ListOrdersInput(BaseModel):
    ...
    status: Optional[str] = Field(
        default=None, description="Filter by order status, e.g. 'draft', 'pending', 'fulfilled'"
    )
    format: Literal["markdown", "json"] = Field(default="markdown", description="Output format")
...
class CalculateShippingInput(BaseModel):
    ...
    items_json: str = Field(
        ..., description="JSON array of order items with catalog_variant_id and quantity"
    )
...
class CreateMockupTaskInput(BaseModel):
    ...
    mockup_style_ids: Optional[str] = Field(
        default=None,
        description=(
            "Comma-separated mockup style IDs. Omit to let Printful "
